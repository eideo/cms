...
public class SearchController extends BaseController{
	...
	private String getSearchString(Map<String,String[]> map,String ip){
		...
		String date = DateTime.toDate("yyyy-MM-dd'T'HH:mm:ss'Z'",new Date());
		String searchString="-titleForIndex:测试 -titleForIndex:lyltest0814 AND publish_date:[* TO "+date+"]";
	...
	private void getSearchOrderByDate(String str,Map<String,String[]> map,String ip,int start,int rows){
		
		//组合除关键字外的所有条件
		String searchString = getSearchString(map,ip);
		
		//查询标题精确匹配数量num
		searchString = searchString+" titleForIndex:\""+str+"\"";
		Map<String, Object> re =indexInfoService.queryIndexInfo(searchString, 0, 0);
		Long num = Long.parseLong((String)re.get("num"));
		//如果start>num,查询精确匹配内容数量num1
		if(start>num){
			
		}
		//如果start<num,但是start+rows>num,查询title精确匹配start=start,row=start+rows-num
		else if(start+rows>num){
			
		}
			
		//如果start>num+num1,查询标题模糊匹配数量num2
		//如果start>num++num1+num2,查询内容模糊匹配数量num3
	}
