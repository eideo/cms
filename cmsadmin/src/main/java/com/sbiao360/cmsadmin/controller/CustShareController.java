 ...
 */
@Controller
@RequestMapping("/sys/custshare")
public class CustShareController extends
	...
	// 查询用户分享页面的表格，包括分页、搜索和排序
	@RequestMapping(value = "/getCustShare", method = { RequestMethod.POST,
			RequestMethod.GET })
	public void getCustShare(HttpServletRequest request,
		...
		custBehavior.setActionType((short) 5);
	...
	// 操作用户分享页面的删除、导出Excel
	@RequestMapping(value = "/operateCustShare", method = { RequestMethod.POST,
			RequestMethod.GET })
	public void operateCustShare(HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		String oper = request.getParameter("oper");
		String id = request.getParameter("id");
		if (oper.equals("del")) {
			String[] ids = id.split(",");
			deleteCustShare(request, response,
					(Long[]) ConvertUtils.convert(ids, Long.class));
		} else if (oper.equals("excel")) {
			response.setContentType("application/msexcel;charset=UTF-8");
			try {
				response.addHeader("Content-Disposition",
						"attachment;filename=custshare.xls");
	...
	// 删除用户分享记录
	@RequestMapping("/deleteCustShare")
	public void deleteCustShare(HttpServletRequest request,
