 ...
 */
@Controller
@RequestMapping("/sys/custclick")
public class CustClickController extends
	...
	// 查询用户点击记录页面的表格，包括分页、搜索和排序
	@RequestMapping(value = "/getCustClick", method = { RequestMethod.POST,
			RequestMethod.GET })
	public void getCustClick(HttpServletRequest request,
	...
	// 操作用户点击记录页面的删除、导出Excel
	@RequestMapping(value = "/operateCustClick", method = { RequestMethod.POST,
			RequestMethod.GET })
	public void operateCustClick(HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		String oper = request.getParameter("oper");
		String id = request.getParameter("id");
		if (oper.equals("del")) {
			String[] ids = id.split(",");
			deleteCustClick(request, response,
					(Long[]) ConvertUtils.convert(ids, Long.class));
		} else if (oper.equals("excel")) {
			response.setContentType("application/msexcel;charset=UTF-8");
			try {
				response.addHeader("Content-Disposition",
						"attachment;filename=custclick.xls");
	...
	// 删除用户点击记录
	@RequestMapping("/deleteCustClick")
	public void deleteCustClick(HttpServletRequest request,
