 ...
 */
@Controller
@RequestMapping("/sys/custaction")
public class CustActionController extends
	...
	// 查询用户关注收藏页面的表格，包括分页、搜索和排序
	@RequestMapping(value = "/getCustAction", method = { RequestMethod.POST,
			RequestMethod.GET })
	public void getCustAction(HttpServletRequest request,
		...
		custBehavior.setActionType((short) 34);
	...
	// 操作用户关注收藏页面的删除、导出Excel
	@RequestMapping(value = "/operateCustAction", method = {
			RequestMethod.POST, RequestMethod.GET })
	public void operateCustAction(HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		String oper = request.getParameter("oper");
		String id = request.getParameter("id");
		if (oper.equals("del")) {
			String[] ids = id.split(",");
			deleteCustAction(request, response,
					(Long[]) ConvertUtils.convert(ids, Long.class));
		} else if (oper.equals("excel")) {
			response.setContentType("application/msexcel;charset=UTF-8");
			try {
				response.addHeader("Content-Disposition",
						"attachment;filename=custaction.xls");
	...
	// 删除用户关注收藏记录
	@RequestMapping("/deleteCustAction")
	public void deleteCustAction(HttpServletRequest request,
