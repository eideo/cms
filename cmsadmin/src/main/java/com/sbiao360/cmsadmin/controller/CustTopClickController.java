 ...
 */
@Controller
@RequestMapping("/sys/custtopclick")
public class CustTopClickController extends
	...
	// 查询用户点击搜索条件页面的表格，包括分页、搜索和排序
	@RequestMapping(value = "/getCustTopClick", method = { RequestMethod.POST,
			RequestMethod.GET })
	public void getCustTopClick(HttpServletRequest request,
		...
		custBehavior.setActionType((short) 7);
	...
	// 操作用户点击搜索条件页面的删除、导出Excel
	@RequestMapping(value = "/operateCustTopClick", method = {
			RequestMethod.POST, RequestMethod.GET })
	public void operateCustTopClick(HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		String oper = request.getParameter("oper");
		String id = request.getParameter("id");
		if (oper.equals("del")) {
			String[] ids = id.split(",");
			deleteCustTopClick(request, response,
					(Long[]) ConvertUtils.convert(ids, Long.class));
		} else if (oper.equals("excel")) {
			response.setContentType("application/msexcel;charset=UTF-8");
			try {
				response.addHeader("Content-Disposition",
						"attachment;filename=custtopclick.xls");
	...
	// 删除用户点击搜索条件记录
	@RequestMapping("/deleteCustTopClick")
	public void deleteCustTopClick(HttpServletRequest request,
