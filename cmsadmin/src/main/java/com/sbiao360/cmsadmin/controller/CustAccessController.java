 ...
 */
@Controller
@RequestMapping("/sys/custaccess")
public class CustAccessController extends
	...
	static {
		...
		sortedMap.put("consumeTime", "CONSUME_TIME");
		sortedMap.put("actionDate", "ACTION_DATE");
	}

	// 查询用户访问页面的表格，包括分页、搜索和排序
	@RequestMapping(value = "/getCustAccess", method = { RequestMethod.POST,
			RequestMethod.GET })
	public void getCustAccess(HttpServletRequest request,
	...
	// 操作用户访问页面的删除、导出Excel
	@RequestMapping(value = "/operateCustAccess", method = {
			RequestMethod.POST, RequestMethod.GET })
	public void operateCustAccess(HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		String oper = request.getParameter("oper");
		String id = request.getParameter("id");
		if (oper.equals("del")) {
			String[] ids = id.split(",");
			deleteCustAccess(request, response,
					(Long[]) ConvertUtils.convert(ids, Long.class));
		} else if (oper.equals("excel")) {
			response.setContentType("application/msexcel;charset=UTF-8");
			try {
				response.addHeader("Content-Disposition",
						"attachment;filename=custaccess.xls");
	...
	// 删除用户访问页面记录
	@RequestMapping("/deleteCustAccess")
	public void deleteCustAccess(HttpServletRequest request,
