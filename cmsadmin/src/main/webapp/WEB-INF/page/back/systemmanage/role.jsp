...
<script type="text/javascript">
        ...
        $('.page-content-area').ace_ajax('loadScripts', scripts, function() {
        	...
        	jQuery(function($) {
        		var grid_selector = "#grid-table-role";
        		...
        		jQuery(grid_selector).jqGrid({
        			...
        			datatype : "json",
        			height : 450,
        			colNames : ['ID', '角色编码', '角色名称', '角色状态', '创建时间', '创建操作员', '修改时间', '修改操作员', '备注', '授权', '操作'],
        			colModel : [ {
        				name : 'id',
        				index : 'id',
        				label : 'ID',
        				...
        				editrules : {required : true, custom : true, custom_func : roleKeyRuleFun}
        			}, {
        				name : 'roleName',
        				index : 'roleName',
        				label : '角色名称',
        				...
        				editrules : {required : true, custom : true, custom_func : roleNameRuleFun}
        			}, {
        				name : 'statusCn',
        				index : 'status',
        				label : '角色状态',
        				...
        				formatoptions : {
        					keys : true,
        					editbutton : <shiro:hasPermission name="${ROLE_KEY}:role:edit">true</shiro:hasPermission><shiro:lacksPermission name="${ROLE_KEY}:role:edit">false</shiro:lacksPermission>,
        					delbutton : <shiro:hasPermission name="${ROLE_KEY}:role:delete">true</shiro:hasPermission><shiro:lacksPermission name="${ROLE_KEY}:role:delete">false</shiro:lacksPermission>
        				}
        			} ],
        		...
        		function roleKeyRuleFun(value, colname) {
        			var reg = /^[A-Za-z0-9]|[_]+$/;
        			if (reg.test(value)) {
        				return [true,""];
        			} else {
        				return [false,"角色编码: 必须是由英文字母、数字或下划线组成的字符串"];
        			}
        		}
        		
        		function roleNameRuleFun(value, colname) {
        			var reg = /^[\u4e00-\u9fa5]{2,60}$/;
        			if (reg.test(value)) {
        				return [true,""];
        			} else {
        				return [false,"角色名称: 请填写正确的角色名称（汉字）"];
        			}
        		}
        	});
        });
