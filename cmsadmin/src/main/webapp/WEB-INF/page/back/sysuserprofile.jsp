				...
				<div class="col-xs-12 col-sm-3 center">
					<div>
						...
						<span class="profile-picture">
							<img id="avatar" class="editable img-responsive" alt="${sysuser.custName}" src="${imgPath}/${sysuser.filePath}" />
						</span>
					...
					<div class="profile-user-info profile-user-info-striped">
						<div class="profile-info-row">
							...
							<div class="profile-info-value">
								<span class="editable" id="custName">${sysuser.custName}</span>
							</div>
						</div>
						<div class="profile-info-row">
							...
							<div class="profile-info-value">
								<span class="editable" id="sex">
									...
									<c:choose>
										<c:when test="${sysuser.sex eq 'N'}">男</c:when>
						...
						<div class="profile-info-row">
							...
							<div class="profile-info-value">
								<span class="editable" id="custEmail" >${sysuser.custEmail}</span>
							</div>
						</div>
						...
						<div class="profile-info-row">
							...
							<div class="profile-info-value">
								<span class="editable" id="mobilePhone">${sysuser.mobilePhone}</span>
							</div>
						</div>
						...
						<div class="profile-info-row">
							...
							<div class="profile-info-value">
								<span class="editable" id="custBirthday">${sysuser.custBirthdayCn}</span>
							</div>
						</div>
					</div>
			...
			<div id="user-profile-3" class="user-profile row">
				<div class="col-sm-offset-1 col-sm-10">
					...
					<form class="form-horizontal">
						<div class="tabbable">
							<input type="hidden" id= "edit_flag" name="edit_flag" value="1">
							<ul class="nav nav-tabs padding-16">
								<li class="active">
									<a data-toggle="tab" href="#edit-basic" onclick="javascript:setEditFlag(1);">
								...
								<li>
									<a data-toggle="tab" href="#edit-password" onclick="javascript:setEditFlag(2);">
							...
							<div class="tab-content profile-edit-tab-content">
								<div id="edit-basic" class="tab-pane in active">
									...
									<div class="row">
										...
										<div class="col-xs-12 col-sm-8">
											<div class="form-group">
												<label class="col-sm-4 control-label no-padding-right" for="form-field-custName">姓名</label>

												<div class="col-sm-8">
													<input class="col-xs-12 col-sm-10" type="text" id="form-field-custName" placeholder="姓名" value="${sysuser.custName}" maxlength="10" />
												</div>
											</div>
											...
											<div class="form-group">
												...
												<div class="col-sm-8">
													<label class="inline">
														<input name="sex" type="radio" class="ace" value="N" <c:choose><c:when test="${sysuser.sex eq 'N'}">checked</c:when></c:choose> />
														<span class="lbl middle">男</span>
													</label>
													&nbsp; &nbsp; &nbsp;
													<label class="inline">
														<input name="sex" type="radio" class="ace" value="Y" <c:choose><c:when test="${sysuser.sex eq 'Y'}">checked</c:when></c:choose> />
											...
											<div class="form-group">
												<label class="col-sm-4 control-label no-padding-right" for="form-field-custBirthday">生日</label>

												<div class="col-sm-8">
													<input class="col-xs-12 col-sm-10" type="text" id="form-field-custBirthday" placeholder="生日" value="${sysuser.custBirthdayCn}" readonly />
												</div>
											</div>											
										</div>
									</div>
									...
									<div class="form-group">
										<label class="col-sm-3 control-label no-padding-right" for="form-field-custEmail">邮箱</label>

										<div class="col-sm-9">
											<span class="input-icon input-icon-right">
												<input type="email" id="form-field-custEmail" value="${sysuser.custEmail}" maxlength="16" />
									...
									<div class="form-group">
										<label class="col-sm-3 control-label no-padding-right" for="form-field-mobilePhone">手机</label>

										<div class="col-sm-9">
											<span class="input-icon input-icon-right">
												<input type="text" id="form-field-mobilePhone" value="${sysuser.mobilePhone}"  maxlength="11"/>
								...
								<div id="edit-password" class="tab-pane">
									...
									<div class="form-group">
										<label class="col-sm-3 control-label no-padding-right" for="form-field-passNew">新密码</label>

										<div class="col-sm-9">
											<input type="password" id="form-field-passNew" maxlength="16" />
										</div>
									</div>
									...
									<div class="form-group">
										<label class="col-sm-3 control-label no-padding-right" for="form-field-passConfirm">确认密码</label>

										<div class="col-sm-9">
											<input type="password" id="form-field-passConfirm" maxlength="16" />
										</div>
									</div>
								</div>
							</div>
...
<script type="text/javascript">
	...
	$('.page-content-area').ace_ajax('loadScripts', scripts, function() {
	    ...
	    jQuery(function($) {
	    	...
	    	$('#form-field-custBirthday').datepicker({
	    	    format: 'yyyy-mm-dd',
	    	    language: 'zh-CN'
	    	});
	            ...
	            $('#avatar').editable({
	                ...
	                url: function(params) {
						...
						deferred.done(function(result) {//success
							...
							if(res.status == 'OK') $(avatar).get(0).src = "${imgPath}" + res.url;
			...
			*/
			...
			$('#submitButton').on('click', function() {
				...
				if(editFlag == 1) {
					var custEmail = $('#form-field-custEmail').val();
					var custName = $('#form-field-custName').val();
					var mobilePhone = $('#form-field-mobilePhone').val();
					if(custName == '') {
						$.gritter.add({
			                title: '系统信息',
			                text: '请输入姓名',
					...
					} else if(custName != '' && !custNameRuleFun(custName)) {
						$.gritter.add({
			                title: '系统信息',
			                text: '请填写真实的姓名（汉字）',
			                class_name: 'gritter-warning gritter-center'
					...
					} else if(custEmail != '' && !custEmailRuleFun(custEmail)) {
						$.gritter.add({
			                title: '系统信息',
			                text: '邮箱格式有误，请填写正确的邮箱',
			                class_name: 'gritter-warning gritter-center'
			            });
					} else if(mobilePhone == '') {
						$.gritter.add({
					...
					} else if(mobilePhone != '' && !mobilePhoneRuleFun(mobilePhone)) {
						$.gritter.add({
			                title: '系统信息',
			                text: '手机格式有误，请填写正确的手机',
			                class_name: 'gritter-warning gritter-center'
			            });
					} else{
						$.ajax({
							...
							data : {
								...
								custEmail : $('#form-field-custEmail').val(),
								custName : $('#form-field-custName').val(),
								mobilePhone : $('#form-field-mobilePhone').val(),
								custBirthday: $('#form-field-custBirthday').val()
							},
					...
					var passNew = $('#form-field-passNew').val();
					var passConfirm = $('#form-field-passConfirm').val();
					if(passNew == '') {
						$.gritter.add({
			                title: '系统信息',
			                text: '请输入新密码',
					...
					} else if(passNew != '' && !passwdRuleFun(passNew)) {
						$.gritter.add({
			                title: '系统信息',
			                text: '密码必须是6-16位以字母、数字、下划线组合的字符串',
			                class_name: 'gritter-warning gritter-center'
			            });
					} else if(passConfirm == '') {
						$.gritter.add({
					...
					} else if(passNew != '' && passNew != passConfirm) {
						$.gritter.add({
			                title: '系统信息',
			                text: '新密码和确认密码不一致，请重新输入',
			                class_name: 'gritter-warning gritter-center'
			            });
					} else if(passNew != '' && passConfirm != '' && passNew == passConfirm) {
						$.ajax({
							dataType : "json",
							url : "${contextPath}" + "/sys/sysuser/resetPassword",
							type : "post",
							data : {
								password : $('#form-field-passNew').val()
							},
							complete : function(xmlRequest) {
								$.gritter.add({
					                title: '系统信息',
